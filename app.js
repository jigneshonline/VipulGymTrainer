const STORE_KEY = "vifit-trainer-dashboard-v10";
...
function renderClientEditor(client) {
  ...
  return `
    ...
    <div class="danger-zone">
      ...
      <p class="subtle">Use reasons. Otherwise future follow-up becomes guesswork.</p>
      <textarea id="clientStatusReason" placeholder="Reason required for disable, discontinue, or delete">${client.statusReason || ""}</textarea>
      <div class="actions">
        <button class="ghost-btn" onclick="setClientStatus('${client.id}', 'active')">Reactivate</button>
        <button class="primary-btn" onclick="setClientStatus('${client.id}', 'active')">Resume</button>
        <button class="ghost-btn" onclick="setClientStatus('${client.id}', 'disabled')">Disable</button>
        <button class="ghost-btn" onclick="setClientStatus('${client.id}', 'discontinued')">Discontinue</button>
...
function setClientStatus(id, nextStatus) {
  ...
  if (nextStatus !== "active" && !reason) {
    alert("Reason is required before disabling or discontinuing a client.");
    return;
  }
  client.status = nextStatus;
  client.statusReason = nextStatus === "active" ? "" : reason;
...
function deleteClient(id) {
  ...
  const client = state.clients.find((item) => item.id === id);
  if (!client) return;
  const confirmed = confirm(`Delete ${client.name}? This removes the client from this prototype. Reason: ${reason}`);
  if (!confirmed) return;
  state.pendingUpdates = state.pendingUpdates.filter((item) => item.clientId !== id);
...
function confirmTrainerHoliday(id) {
  const client = state.clients.find((item) => item.id === id);
  if (!client) return;
  const message = client.holidayByTrainer
    ? "Remove trainer holiday and reopen today's workout?"
    : `Mark trainer holiday for ${client.name}? Client can still continue self-workout, so don't break their streak unless needed.`;
  if (!confirm(message)) return;
  toggleHoliday(id, "holidayByTrainer");
}

function confirmClientHolidayByTrainer(id) {
  const client = state.clients.find((item) => item.id === id);
  if (!client) return;
  const message = client.holidayByClient
    ? "Remove client holiday and bring them back to today's plan?"
    : `Mark client holiday for ${client.name}? Are you sure? A planned rest is fine, but avoid breaking the streak casually.`;
  if (!confirm(message)) return;
  toggleHoliday(id, "holidayByClient");
}
